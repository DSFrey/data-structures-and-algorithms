...
const validateEmail = (email) => /^((\w+)|(\w+\.\w+))@\w+\.((net)|(com)|(org))$/.test(email);
...
const validatePhoneNumber = (phoneNumber) => /^((\d{3})|(\(\d{3}\)))-?\s?((\d{3})|(\(\d{3}\)))-?\s?((\d{4})|(\(\d{4}\)))$/.test(phoneNumber);
...
const findTagNames = elements => elements.reduce((total,string) => [...total, ...string.match(/\/\w+/g)],[]);
