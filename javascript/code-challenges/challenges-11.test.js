...
function transformToLis(obj) {
  return Object.entries(obj).map(property => `<li>${property[0]}: ${property[1]}</li>`);
}
...
const count = (target, input) => input.reduce((sum, subArray) => sum + subArray.filter(entry => entry === target).length, 0);
...
const totalSum = (input) => input.reduce((sum, subArray) => sum + subArray.reduce((sum, entry) => sum + entry, 0), 0);
...
const divisibleByFiveTwoToThePower = (input) => input.map(subArray => subArray.filter(entry => entry % 5 === 0 && typeof entry === 'number').map(entry => Math.pow(2, entry)));
