...
const addQuestion = (arr) => arr.map(originalString => originalString + '?');
...
const extractAbilities = (arr) => arr.map(slot => slot.ability.name);
...
const extractStats = (arr) => arr.map(statEntry => ({name: statEntry.stat.name, total: (statEntry.baseStat + statEntry.effort)}));
