...
const addValues = (arr) => arr.reduce((previous, current) => previous + current, 0);
...
const addPurchases = (arr) => arr.reduce((previous, current) => previous + current.purchasePrice, 0);
...
const countNumberOfElements = (arr) => arr.reduce(previous => previous + 1, 0);
...
const returnNames = (arr) => arr.reduce((previous, current) => [...previous, current.name], []);
...
const reversedString = (str) => str.split('').reduce((previous, current) => current + previous, '');
...
const countNumberOfChildren = (arr) => arr.reduce((previous, current) => 'children' in current ? previous + current.children.length : previous, 0);
...
const calculateAverage = (arr) => {
  let reduction = arr.reduce((previous, current) => ({ count: previous.count + 1, sum: previous.sum + current }), { count: 0, sum: 0 });
...
const countPrimeNumbers = (arr) => arr.reduce((previous, current) => isPrime(current) ? previous + 1 : previous, 0);
...
const extractStat = (statName, arr) => arr.reduce((previous, current) => current.stat.name === statName ? current : previous, '');
...
const extractChildren = (arr) => arr.filter((character) => character.name.includes('a')).reduce((previous, current) => 'children' in current ? [...previous, ...current.children] : previous, []);
